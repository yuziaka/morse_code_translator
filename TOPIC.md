...
А для этого всего лишь нужно оформить наш код в виде пакета и опубликовать его на pypi.org, но есть
...
Множество Python пакетов распространяются за лицензиями которые позволяют нам использоваться ими 
бесплатно.
А к примеру MIT лицензия дает нам возможность не только использовать, но и модифицировать код, при 
...
Как пример `boto3` для работы с сервисами AWS, клиентская библиотеки для Stripe и так далее.
...
Для этого нам нужно упаковать его в пакет и опубликовать в PyPI.
Но с чего же начать? 

### Документация

Каждый из нас хотя бы раз в жизни сталкивался с необходимостью использования внешнего пакета. 
И если у этого пакета есть хорошая документация с примерами использования это ускоряет его
интеграцию несколько раз. 
...
для меня. 
Для того что бы разобрать, что это за инструмент можно пройти по пути из
...
Сначала нам мы импортируем из модуля `setuptools` несколько функций.
Читаем `README.md`.
...
распространенным решением и на мой взгляд удачным решением для этого есть README.md
...
Но для нас более удобным будет создать его вручную, для этого создаем две директории `.github` и 
...
Но перед тем как мы начнем писать наш Action который будет тестировать, собирать и заливать
нашу новую версию пакете в PyPI, нам нужно получить токен для публикации в PyPI.
...
Но у есть одна особенность, а именно то что это задача запускается не один раз.
...
И как итог наши тесты будут запущены во всех поддерживаемых нами окружениях.
...
Первая создаст тег, при этом даст ема название используя версию нашего пакета которая указана в 
`setup.py`:
...
А вот так это выглядит в github:
